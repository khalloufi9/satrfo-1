...
import PalyersList from "./PalyersList";
// import jwt_decode from 'jwt_decode';
const AddPlayer = () => {
  const [nameJoueur, setNameJoueur] = useState("");
  ...
  /** end states */
  useEffect(() => {
    const equipeId = localStorage.getItem("equipe");
    setEquipe(equipeId)
  }, [])
  // Upload image
  const upload = ({ target: { files } }) => {
    ...
    data.append("nameJoueur", nameJoueur);
    data.append("poste", poste);
    data.append("equipe", equipe);
    setFormData(data);
  };
  ...
  const handleSubmit = (e) => {
    ...
    axios
      .post("http://localhost:5000/joueur/add", formData, options)
      .then((res) => {
              ...
              <div className="d-flex justify-content-center align-items-center flex-column">
                ...
                >
                  <input
                    ...
                    value={nameJoueur}
                    onChange={(e) => setNameJoueur(e.target.value)}
                  />
                  <input
                    type="text"
                    readonly
